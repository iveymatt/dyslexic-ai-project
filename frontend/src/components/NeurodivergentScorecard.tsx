    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-cyan-50 to-blue-50 rounded-xl p-6 border border-cyan-200">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h2 className="text-2xl font-bold text-earth-800">Neurodivergent Thinking Support</h2>
        </div>
        <p className="text-earth-600 leading-relaxed">
          How well does <span className="font-semibold text-earth-800">{toolName}</span> support
      ...
      <div className="bg-white rounded-xl p-6 border border-earth-200">
        <h3 className="text-lg font-bold text-earth-800 mb-6">Score Breakdown</h3>
        ...
        <div className="space-y-5">
                ...
                <div className="flex items-start justify-between mb-2">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <Icon size={18} className="text-earth-500" />
                      <span className="text-sm font-semibold text-earth-700">{dim.label}</span>
                    </div>
                    <p className="text-xs text-earth-400">{dim.description}</p>
                  </div>
                  ...
                  {[...Array(10)].map((_, i) => (
                    <div
                      ...
                      className={`h-2 flex-1 rounded-sm ${
                        i < barCount ? getScoreBgColor(value) : 'bg-earth-100'
                      }`}
                    />
                  ))}
                </div>
          ...
          {/* Divider */}
          <div className="border-t border-earth-200 pt-5">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <Target size={22} className="text-cyan-500" />
                <span className="text-lg font-bold text-earth-800">OVERALL</span>
              </div>
              ...
              {[...Array(10)].map((_, i) => (
                <div
                  ...
                  className={`h-3 flex-1 rounded-sm ${
                    i < Math.round(scores.overall) ? getScoreBgColor(scores.overall) : 'bg-earth-100'
                  }`}
                />
              ))}
            </div>
      ...
      <div className="grid md:grid-cols-2 gap-4">
        {/* Best At */}
        <div className="bg-white rounded-xl p-5 border border-earth-200">
          <div className="flex items-center gap-2 mb-4">
            <Check className="text-green-500" size={20} />
            <h3 className="text-base font-bold text-earth-800">Best At</h3>
          </div>
          <ul className="space-y-2">
            {bestAt.map((item, index) => (
              <li key={index} className="flex items-start gap-2 text-sm text-earth-600">
        ...
        <div className="bg-white rounded-xl p-5 border border-earth-200">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-base font-bold text-earth-800">Weak At</h3>
          </div>
          <ul className="space-y-2">
      ...
      <div className="grid md:grid-cols-2 gap-4">
        ...
        <div className="bg-white rounded-xl p-5 border border-earth-200">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-base font-bold text-earth-800">Perfect For</h3>
          </div>
          <ul className="space-y-2">
        ...
        <div className="bg-white rounded-xl p-5 border border-earth-200">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-base font-bold text-earth-800">Not Ideal For</h3>
          </div>
          <ul className="space-y-2">
      ...
      <div className="bg-gradient-to-r from-blue-900/20 to-purple-900/20 rounded-xl p-6 border border-blue-700/30">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h3 className="text-lg font-bold text-earth-800 mb-2">Summary</h3>
            <p className="text-earth-600 leading-relaxed">
              <span className="font-semibold text-earth-800">{toolName}</span> scores{' '}
